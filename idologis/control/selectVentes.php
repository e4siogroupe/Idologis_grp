...
if(isset($_POST['tri']) && $_POST['tri'] == "ASC") {
	$request = "SELECT reference, type.libelle AS type, secteur.libelle AS secteur, surface, prix, chambres, surface_terrain, classe_energetique, les_plus ".
	"FROM vente ".
	...
	$request = "SELECT reference, type.libelle AS type, secteur.libelle AS secteur, surface, prix, surface_terrain, classe_energetique, les_plus ".
	"FROM vente ".
	"INNER JOIN type ON type.id = vente.id_type ".
	"INNER JOIN secteur ON secteur.id = vente.id_secteur ".
	"ORDER BY prix DESC";
